...
render_admin_header('باشگاه امتیاز و پاداش', 'p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6', '', '');
?>
            ...
            <section class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                <div><p class="overline text-indigo-600">مدیریت سادهٔ امتیاز و پاداش</p><h1 class="h-1 text-slate-900 mt-2">باشگاه امتیاز و پاداش</h1><p class="text-sm text-slate-500 mt-2 max-w-2xl leading-6">سه کار اصلی را انجام دهید: روش‌های امتیازدهی را روشن کنید، پاداش و کدهای تخفیف را بسازید و موجودی مشتریان را بررسی کنید.</p></div>
                <span class="badge <?= $moduleOn ? 'badge-success' : 'badge-warning' ?>"><?= $moduleOn ? 'ماژول فعال' : 'ماژول غیرفعال' ?></span>
            </section>

            <section class="grid grid-cols-2 lg:grid-cols-4 gap-3" aria-label="خلاصهٔ مدیریتی Gamification">
                <div class="card p-4"><span class="text-xs text-slate-500">روش‌های امتیازدهی فعال</span><strong class="block text-2xl text-indigo-600 mt-2 tabular-nums" dir="ltr"><?= $activeRuleCount ?></strong></div>
                <div class="card p-4"><span class="text-xs text-slate-500">مشتریان دارای کیف امتیاز</span><strong class="block text-2xl text-emerald-600 mt-2 tabular-nums" dir="ltr"><?= count(array_filter($customerBalances, static fn (array $row): bool => (int) $row['total_earned'] > 0)) ?></strong></div>
                <div class="card p-4"><span class="text-xs text-slate-500">کدهای آمادهٔ مصرف</span><strong class="block text-2xl text-amber-600 mt-2 tabular-nums" dir="ltr"><?= $availableCouponCount ?></strong></div>
                <div class="card p-4"><span class="text-xs text-slate-500">پاداش‌های صادرشده</span><strong class="block text-2xl text-violet-600 mt-2 tabular-nums" dir="ltr"><?= $totalIssuedCount ?></strong></div>
            </section>

            <section class="card p-5 sm:p-6 border-2 <?= $moduleOn ? 'border-emerald-200' : 'border-amber-200' ?>" aria-labelledby="module-title">
            ...
            <section class="card p-5 sm:p-6 space-y-4" aria-labelledby="rules-title">
